...
RSP se retrouve donc décalé de 16 octets, soit l'espace nécessaire pour stocker nos variables `a` et `b`. Pour y accéder, le code machine utilisera généralement un index, basé RBP ou RSP et un offset, soit un code proche de :
```asm
mov [rsp], 0    ; a = 0
mov [rsp+8], 10 ; b = 10
```
