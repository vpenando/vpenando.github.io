...
### Instructions
Le 6502 dispose d'[un jeu d'instructions](http://obelisk.me.uk/6502/reference.html#INC) assez riche permettant d'accéder aux différents registres. L'instruction `lda` ('**L**oa**D** into **A**ccumulator') permet d'écrire dans A, tandis que `ldx` et `ldy` écrivent respectivement dans X et Y.
Exemple :
```asm
...
Il est possible de stocker les valeurs des différents registres en mémoire via les instructions `sta` ('**ST**ore **A**ccumulator), `stx` et `sty` :
```asm
