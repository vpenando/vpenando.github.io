...
Je fais également un peu de fonctionnel avec OCaml / F#, et j'ai eu l'occasion d'utiliser Elm sur mon temps libre. Ces langages disposent de types proposant des valeurs pseudo-nulles. Exemple :
```ml
(* OCaml / F# *)
...
let print_if_exists optionalValue =
  ...
  ;;
```
Le type `option` correspond, comme son nom l'indique, à une valeur optionnelle. Il accepte de ce fait deux "types" de valeurs : `None` et `Some xxx`, avec `xxx` une valeur quelconque, telle que "Hello, world!" dans l'exemple ci-dessus.
